    ...
    */
    ...
    while (1) {
        ...
        if (nal_length > 0) {
            if ((slice_type == 0) && (frame_num != (last_frame_num + 1))) {
                LOGD("Error, wrong number, FIXME FIXME");
                {
                    ...
                    snprintf(temp, 512, "ST=%d,  FN=%d, NAL=%d, LFN=%d, FNL=%d, 0x%02x%02x%02x%02x",
                             ...
                             video_check_pattern[5], video_check_pattern[6], video_check_pattern[7],
                             video_check_pattern[8]);
                    //LOGD(temp);
                }
            ...
            if (fillBuffer(&buf[video_check_pattern.size()],
                           nal_length - (video_check_pattern.size() - 4)) < 0)
                break;
            /*
            ...
            flvPackager->resetBuffer();
            */
            mediaBuffer->PushBuffer(buf, nal_length + 4, frame_count * 88,
                                    slice_type ? MEDIA_TYPE_VIDEO_KEYFRAME : MEDIA_TYPE_VIDEO);
...
void MediaStreamer::doCapture() {
    ...
    while (1) {
        ...
        if (buf[0] == 0x00) {
            ...
            mediaBuffer->PushBuffer(buf, vpkg_len, last_ts,
                                    slice_type ? MEDIA_TYPE_VIDEO_KEYFRAME : MEDIA_TYPE_VIDEO);
#else
            vseq ++;
            ...
            static const unsigned char packed_size[16] = {12, 13, 15, 17, 19, 20, 26, 31, 5, 0, 0,
                                                          0, 0, 0, 0, 0};
...
int MediaStreamer::checkSingleSliceNAL(const std::deque<unsigned char> &pattern, int &slice_type,
                                       unsigned int &frame_num) {
    ...
    if (frame_num_length == -1) {
        ...
        unsigned int bits =
                (pattern[5] << 24) + (pattern[6] << 16) + (pattern[7] << 8) + pattern[8];
